...
describe Property::Searcher do
  ...
  context 'searcher: rentability(1) ' do
    subject { default_searcher }

    let(:default_searcher) do
      c = Property::Searcher.new rentability: 1
      puts c.inspect
      c
    end
    ...
    describe '.search' do
      ...
      specify do
        subject.criteria.where_criteria.should == {"address.country_code"=>"DK", "rentability"=>{"$gte"=>1, "$lte"=>2}}
      end
      ...
      let(:range) { Property::Criteria::Mapper.map_for(:rentability) }
      ...
      it 'should have only search results that match criteria' do
        ...
        search_result.each do |found|
          puts "FOUND: #{found}"
          puts "Rating: #{found.rentability}"
          
          found.rentability.should <= range[1].max          
        end
